...
const cloudinaryFolder = process.env.CLOUDINARY_WILDLIFE_FOLDER
  || process.env.CLOUDINARY_WILDLIFE_PREFIX
  || 'images/wildlife';
...
function isWildlifeResource(resource) {
...
async function syncWildlife() {
  ...
  const resources = (await fetchAllResources()).filter(isWildlifeResource);
  const newItems = resources
    .map(toPortfolioItem)
    .filter(item => !existingIds.has(item.public_id));

  if (!newItems.length) {
    console.log(`No new wildlife images found under ${cloudinaryFolder}.`);
    return;
  }
  ...
  console.log(`Added ${newItems.length} wildlife image${newItems.length === 1 ? '' : 's'} to images.yml.`);
}

syncWildlife().catch(error => {
  console.error(`Wildlife sync failed: ${error.message}`);
  process.exitCode = 1;
});
...
const cloudinaryFolder = process.env.CLOUDINARY_ASTRO_FOLDER
...
function isAstroResource(resource) {
...
async function syncAstro() {
  ...
  const resources = (await fetchAllResources()).filter(isAstroResource);
  const newItems = resources
    .map(toPortfolioItem)
    .filter(item => !existingIds.has(item.public_id));

  if (!newItems.length) {
    console.log(`No new astro images found under ${cloudinaryFolder}.`);
    return;
  }
  ...
  console.log(`Added ${newItems.length} astro image${newItems.length === 1 ? '' : 's'} to images.yml.`);
}

syncAstro().catch(error => {
  console.error(`Astro sync failed: ${error.message}`);
  process.exitCode = 1;
});
